...
const BulkOffersMonitor: React.FC = () => {
  ...
  const columns: GridColDef<BulkOffer>[] = [
    ...
    },
    {
      ...
      align: 'center',
      headerAlign: 'center',
      renderCell: (params: GridRenderCellParams<BulkOffer>) => {
  ...
  // Admin-only hard delete: removes the negotiation from the system so it
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      await api.delete(`/bulk-offers/admin/${id}`);
      return id;
    },
    onSuccess: (id) => {
      queryClient.invalidateQueries({ queryKey: ['admin', 'bulk-offers'] });
      queryClient.removeQueries({ queryKey: ['admin', 'bulk-offers', id] });
    },
  });
